...
### 10-2. 시뮬레이션에서 WCC가 TR을 앞서는 3가지 메커니즘
...
TR이 WCC보다 우위를 가지려면 "TR이 추가로 얻는 주가 상승분" > "WCC가 얻는 옵션 프리미엄"이 되어야 합니다. 즉, 시장이 콜옵션 행사가를 빠르게 돌파하는 **강력한 상승장**에서만 TR이 유리합니다.

#### ② 2025–2026 한국 시장 환경: 커버드콜의 스윗 스팟

측정 기간 동안 KOSPI 200 관련 ETF는 약 **19–20% 수준의 완만한 상승**을 기록했습니다. 이는 커버드콜 전략에 최적의 환경입니다.

```
시장 국면별 커버드콜 vs 토탈 리턴 성과:

급등장  │      TR ▲▲▲  WCC ▲   → TR 우세 (옵션 행사가에 상승 제한)
완만 상승│      TR ▲▲   WCC ▲▲▲ → WCC 우세 ← 현재 구간
횡보    │      TR ≈    WCC ▲   → WCC 우세 (프리미엄만 수취)
하락    │      TR ▼▼   WCC ▼   → WCC 우세 (프리미엄이 하락 완충)
```
...
이 과정이 **13개월 반복**되면서 주식 수가 꾸준히 늘어납니다. 이는 TR의 내부 복리와 구조적으로 동일하지만, WCC는 옵션 프리미엄이라는 **외부 수익원**이 추가로 복리에 기여합니다.

TR은 이미 모든 배당을 NAV 내부에 재투자하므로 yfinance의 `Dividends` 컬럼이 0입니다. 따라서 시뮬레이션 상에서 TR은 순수한 주가 상승만 반영되는 반면, WCC는 주가 상승 + 월별 분배금 재투자가 이중으로 기여합니다.
...
WCC의 약 94만 원 우위는 **월평균 약 7.2만 원의 옵션 프리미엄 순기여분**에 해당합니다 (13개월 기준).

### 10-4. WCC 전략의 한계와 주의사항

| 주의사항 | 설명 |
|----------|------|
| **강세장 제한** | KOSPI 200이 콜옵션 행사가를 돌파하는 국면에서는 TR보다 수익이 낮음 |
...
```
✅ 커버드콜(WCC)이 유리한 경우
   - 시장이 완만하게 상승하거나 횡보할 것으로 예상될 때
   - 정기적인 현금 흐름(분배금)이 필요한 투자자
   - 하락 위험을 부분적으로 완충하고 싶을 때

✅ 토탈 리턴(TR)이 유리한 경우
   - 시장이 강력하게 상승할 것으로 예상될 때 (강세장)
